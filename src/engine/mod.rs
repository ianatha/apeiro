...
impl Engine {
    ...
    pub async fn step_fs_process(&self, pid: &String, js_stmt: String) -> Result<String> {
        ...
        let (new_state, new_snpashot) = self.step_process(src, state, snapshot, js_stmt).await?;
    ...
    async fn step_process_first(&self, src: Vec<u8>, js_stmt: String) -> Result<(String, Vec<u8>)> {
