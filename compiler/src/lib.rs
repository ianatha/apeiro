...
use swc_core::{
    common::chain,
    ecma::{ast::EsVersion, transforms::base::pass::noop},
};
...
fn top_level_from_program(parsed: &swc_core::ecma::ast::Program) -> Option<String> {
    match parsed.as_module() {
        Some(module) => module.body.iter().find_map(|stmt| match stmt {
            ...
            swc_core::ecma::ast::ModuleItem::ModuleDecl(
                swc_core::ecma::ast::ModuleDecl::ExportDefaultDecl(default_decl),
...
pub fn extract_export_name(input: String) -> String {
    ...
    if let Ok((_source_file, parsed)) = swc_core::common::GLOBALS
