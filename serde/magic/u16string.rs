...
impl FromV8 for U16String {
    fn from_v8(
        ...
        let v8str = v8::Local::<v8::String>::try_from(value).map_err(|_| Error::ExpectedString)?;
        ...
        unsafe {
            buffer.set_len(len);
            let written = v8str.write(scope, &mut buffer, 0, v8::WriteOptions::NO_NULL_TERMINATION);
            assert!(written == len);
        }
        Ok(buffer.into())
    }
