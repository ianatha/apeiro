...
fn magic_buffer() {
    v8_do(|| {
        ...
        let zbuf: serde_v8::ZeroCopyBuf = serde_v8::from_v8(scope, v8_array).unwrap();
        ...
        let v8_array = js_exec(scope, "new Uint8Array([1,2,3,4,5])");
        let zbuf: Option<serde_v8::ZeroCopyBuf> = serde_v8::from_v8(scope, v8_array).unwrap();
        assert_eq!(&*zbuf.unwrap(), &[1, 2, 3, 4, 5]);

        // Observe mutation in JS
        let v8_array = js_exec(scope, "new Uint8Array([1,2,3,4,5])");
        let mut zbuf: serde_v8::ZeroCopyBuf = serde_v8::from_v8(scope, v8_array).unwrap();
        ...
        let zbuf: Result<serde_v8::ZeroCopyBuf> = serde_v8::from_v8(scope, v8_array);
        assert!(zbuf.is_err());
